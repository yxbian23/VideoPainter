...
def main_worker(args):
    ...
    if args.caption_path:
        ...
        print(f"Loading caption from: {args.caption_path}")
    ...
    for index, items in enumerate(test_loader):
        ...
        if "long" in args.caption_path:
            if "edit" not in args.caption_path:
        ...
        # if "000000207786_8632822" not in video_name:
        #     continue
        ...
        print(f"max_video_length: {args.max_video_length}")
        ...
        if caption_df is not None:
            if args.dataset == 'davis':
                ...
                video_inpainting_prompt = video_row['Edited_video_caption']
                image_inpainting_prompt = video_row['Edited_image_caption']
                Editing_instruction = video_row['Editing_instruction']
        else:
            raise NotImplementedError
        if len(comp_frames) < args.max_video_length:
            ...
            pipe_img_inpainting = FluxFillPipeline.from_pretrained(args.img_inpainting_model, torch_dtype=torch.bfloat16).to("cuda")
