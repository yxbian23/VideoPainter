...
def main_worker(args):
    ...
    result_path = os.path.join(args.save_addr, f'VPBench_wo_branch_{args.dataset}_dilate_{args.dilate_size}')
    if args.replace_gt:
        result_path = os.path.join(args.save_addr, f'VPBench_wo_branch_{args.dataset}_dilate_{args.dilate_size}_w_replace_gt')
    else:
        result_path = os.path.join(args.save_addr, f'VPBench_wo_branch_{args.dataset}_dilate_{args.dilate_size}_wo_replace_gt')
    ...
    if args.id_pool_resample_learnable_path is None:
        ...
        pipe.load_lora_weights(
            args.id_pool_resample_learnable_path, 
    ...
    for index, items in enumerate(test_loader):
        ...
        if os.path.exists(save_comp_path):
            ...
            if existing_frames >= args.max_video_length:
                print(f"Skipping generation of {video_name} - {existing_frames} frames already exist (target: {args.max_video_length})")
        ...
        if len(comp_frames) < args.max_video_length:
            ...
            if len(video_PIL) < args.max_video_length:
                print(f"video length is less than {args.max_video_length}, using {len(video_PIL) - len(video_PIL) % 4 + 1} frames...")
                continue
            ...
            inpaint_outputs = pipe(
                ...
                stride= 49, # int(frames - down_sample_fps), frames,
                prev_clip_weight=0.0,
                id_pool_resample_learnable=True if args.id_pool_resample_learnable_path is not None else False,
                output_type="np"
            ).frames[0]
        ...
        comp_frames = np.stack(comp_frames)  # [T,H,W,3]
        ...
        if args.save_results:
            save_frame_path = os.path.join(result_path, video_name)
            save_comp_path = os.path.join(save_frame_path, 'comp_frames')
            save_ori_path = os.path.join(save_frame_path, 'ori_frames')
            save_mask_path = os.path.join(save_frame_path, 'masks')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--id_pool_resample_learnable_path', type=str, default=None,
