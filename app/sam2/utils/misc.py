...
def _process_img_as_tensor(frame, image_size):
...
def process_loaded_video_frames(
    frames,
    image_size,
    offload_video_to_cpu,
    img_mean=(0.485, 0.456, 0.406),
    ...
    images = torch.zeros(num_frames, 3, image_size, image_size, dtype=torch.float32)
    # for n, frame in enumerate(tqdm(frames, desc="Processing frames", position=0, leave=True)):
    for n, frame in enumerate(frames):
        images[n], video_height, video_width = _process_img_as_tensor(frame, image_size)
    if not offload_video_to_cpu:
