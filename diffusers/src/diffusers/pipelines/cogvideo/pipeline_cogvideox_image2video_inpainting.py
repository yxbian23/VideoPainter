...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import CogVideoXImageToVideoInpaintPipeline
        >>> from diffusers.utils import export_to_video, load_image

        >>> pipe = CogVideoXImageToVideoInpaintPipeline.from_pretrained("THUDM/CogVideoX-5b-I2V", torch_dtype=torch.bfloat16)
        >>> pipe.to("cuda")
...
class CogVideoXImageToVideoInpaintPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        init_video = self.video_processor.preprocess_video(original_video, height=height, width=width).to(
            device, dtype=prompt_embeds.dtype
        )
        ...
        # latents, image_latents = self.prepare_latents_original(
        #     image,
        #     batch_size * num_videos_per_prompt,
        #     latent_channels,
        #     num_frames,
        ...
        mask_condition = self.masked_video_processor.preprocess_video(masked_video, height=height, width=width).to(device)
