...
class CogVideoXInpaintPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    Pipeline for text-to-video inpainting using CogVideoX.
    ...
    def check_inputs(
        ...
        if video is None:
            raise ValueError(
                f"Video is required for inpainting."
            )
        if masked_video is None:
            raise ValueError(
                f"masked_video is required for inpainting."
            )
