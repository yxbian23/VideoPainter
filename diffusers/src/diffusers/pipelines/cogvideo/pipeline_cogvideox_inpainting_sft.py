...
from ...models import AutoencoderKLCogVideoX, CogVideoXTransformer3DModel, CogVideoXTransformer3DInpaintModel
...
class CogVideoXSFTInpaintPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    model_cpu_offload_seq = "text_encoder->branch->vae"
    ...
    def __init__(
        ...
        branch: CogVideoXTransformer3DInpaintModel,
    ):
        super().__init__()

        self.register_modules(
            tokenizer=tokenizer, text_encoder=text_encoder, vae=vae, scheduler=scheduler, branch=branch
        )
    ...
    def fuse_qkv_projections(self) -> None:
        ...
        self.branch.fuse_qkv_projections()
    ...
    def unfuse_qkv_projections(self) -> None:
        ...
        if not self.fusing_transformer:
            ...
            self.branch.unfuse_qkv_projections()
    ...
    def _prepare_rotary_positional_embeddings(
        ...
        grid_height = height // (self.vae_scale_factor_spatial * self.branch.config.patch_size)
        grid_width = width // (self.vae_scale_factor_spatial * self.branch.config.patch_size)
        base_size_width = 720 // (self.vae_scale_factor_spatial * self.branch.config.patch_size)
        base_size_height = 480 // (self.vae_scale_factor_spatial * self.branch.config.patch_size)

        grid_crops_coords = get_resize_crop_region_for_grid(
            (grid_height, grid_width), base_size_width, base_size_height
        )
        freqs_cos, freqs_sin = get_3d_rotary_pos_embed(
            embed_dim=self.branch.config.attention_head_dim,
    ...
    def __call__(
        ...
        height = height or self.branch.config.sample_size * self.vae_scale_factor_spatial
        width = width or self.branch.config.sample_size * self.vae_scale_factor_spatial
        ...
        # 5. Prepare latents.
        latent_channels = self.branch.config.in_channels
        ...
        if return_video_latents:
            latents, noise, video_latents = latents_outputs
        else:
        ...
        image_rotary_emb = (
            ...
            if self.branch.config.use_rotary_positional_embeddings
            else None
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                # branch block
                # mask = 1.0 - mask
                latent_branch_input = torch.cat([latent_model_input, masked_video_latents, mask[:, :, :1, :, :]], dim=-3)
                
                noise_pred = self.branch(
                    hidden_states=latent_branch_input,
