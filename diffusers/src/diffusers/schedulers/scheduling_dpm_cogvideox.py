...
class CogVideoXDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def compute_pred_original_sample(
        self,
        model_output: torch.Tensor,
        sample: torch.Tensor,
        timestep: int
        ...
        """
        Compute the predicted original sample x_0 from the model output and the current noisy sample.

        Args:
            model_output (`torch.Tensor`): The predicted noise from the model.
            sample (`torch.Tensor`): The current noisy sample.
            timestep (`int`): The current timestep in the diffusion chain.

        Returns:
            `torch.Tensor`: The predicted original sample x_0.
        """
        alpha_prod_t = self.alphas_cumprod[timestep]
        beta_prod_t = 1 - alpha_prod_t
