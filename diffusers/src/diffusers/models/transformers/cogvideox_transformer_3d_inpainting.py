...
class CogVideoXTransformer3DInpaintModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def from_transformer(
        ...
        if load_weights_from_transformer:
            ...
            branch.norm_final.load_state_dict(transformer.norm_final.state_dict())
            branch.norm_out.load_state_dict(transformer.norm_out.state_dict())
            branch.proj_out.load_state_dict(transformer.proj_out.state_dict())
    ...
    def forward(
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            if self_guidance_hidden_states is not None:
                # 1. adding
                # hidden_states += self_guidance_hidden_states[i]
                # 2. swap based on mask
                hidden_states = torch.where(masks == 1, self_guidance_hidden_states[i], hidden_states)
            ...
            if branch_block_samples is not None:
                if not add_first:
                    ...
                    # print(f"adding {i // interval_control + 1} - {len(branch_block_samples)} for {i + 1} - {len(self.transformer_blocks)}")
                else:
                    # print(f"adding first {i} - {len(branch_block_samples)}")
